...
const hub = fs.readFileSync(path.join(ROOT, 'content', 'features', 'finance_hub', 'operations.js'), 'utf8').replace(/\r\n/g, '\n');
const sharedUi = fs.readFileSync(path.join(ROOT, 'content', 'features', 'finance_hub', 'shared_ui.js'), 'utf8').replace(/\r\n/g, '\n');
const css = fs.readFileSync(path.join(ROOT, 'css', 'content_styles.css'), 'utf8').replace(/\r\n/g, '\n');

assert.match(sharedUi, /Math\.round\(\(value \+ Number\.EPSILON\) \* 100\) \/ 100/, 'money formatter must preserve cents');
assert.match(sharedUi, /maximumFractionDigits:\s*2/, 'money formatter must render up to two decimals');
